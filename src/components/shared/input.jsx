...
const Input = ({
  ...
  const getInputIcon = () => {
    return type === "password" ? (showPassword ? eyeClose : eyeOpen) : icon;
  };
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      {label && <label className={`extra-small text-black font-medium ${labelClass}`}>{label}</label>}
      <div
        className={`relative flex items-center border-[1px] border-solid border-[#247BA0]  rounded-[8px]  p-3 h-10   ${
